...
// getPlatformConfigManifest returns the sample PlatformConfig manifest
// based upon API Version input.
func getPlatformConfigManifest(i *cmdinit.InitSubCommand) (string, error) {
	apiVersion := i.APIVersion
	if apiVersion == "" || apiVersion == "latest" {
		return capabilities.PlatformConfigLatestSample, nil
	}

	// generate a map of all versions to samples for each api version created
	manifestMap := map[string]string{
		"v1alpha1": v1alpha1platformconfig.Sample(i.RequiredOnly),
...
func InitPlatformConfig(i *cmdinit.InitSubCommand) error {
	manifest, err := getPlatformConfigManifest(i)
	if err != nil {
		return fmt.Errorf("unable to get manifest for PlatformConfig; %w", err)
	}
