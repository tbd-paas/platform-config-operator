...
func NewPlatformConfigSubCommand(parentCommand *cobra.Command) {
	versionCmd := &cmdversion.VersionSubCommand{
		...
		VersionFunc:  VersionPlatformConfig,
...
func VersionPlatformConfig(v *cmdversion.VersionSubCommand) error {
	apiVersions := make([]string, len(capabilities.PlatformConfigGroupVersions()))

	for i, groupVersion := range capabilities.PlatformConfigGroupVersions() {
