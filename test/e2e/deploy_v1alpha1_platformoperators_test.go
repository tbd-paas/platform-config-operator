...
// deployv1alpha1PlatformOperators tests
func deployv1alpha1PlatformOperatorsChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(platformoperators.CreateFuncs) == 0 {
		return nil
	}

	workload, err := platformoperators.ConvertWorkload(tester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := platformoperators.Generate(*workload, nil, nil)
	if err != nil {
...
func deployv1alpha1PlatformOperatorsNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &deployv1alpha1.PlatformOperators{},
		sampleManifestFile: "../../config/samples/deploy_v1alpha1_platformoperators.yaml",
		getChildrenFunc:    deployv1alpha1PlatformOperatorsChildrenFuncs,
		logSyntax:          "controllers.deploy.PlatformOperators",
	}
}

func (tester *E2ETest) deployv1alpha1PlatformOperatorsTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_deployv1alpha1PlatformOperators() {
	tester := deployv1alpha1PlatformOperatorsNewHarness("")
	tester.deployv1alpha1PlatformOperatorsTest(testSuite)
}
